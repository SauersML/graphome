...
pub fn compute_eigenvalues_and_vectors_sym(
    ...
    let faer_matrix = Mat::<f64>::from_fn(rows, cols, |i, j| laplacian[(i, j)]);
    let decomposition = SelfAdjointEigen::new(faer_matrix.as_ref(), Side::Lower)
        .map_err(|err| io::Error::new(io::ErrorKind::Other, format!("{:?}", err)))?;

    let eigenvalues = Array1::from(
        decomposition
            .S()
            .column_vector()
            .iter()
...
pub fn save_matrix_to_csv<P: AsRef<Path>>(matrix: &Array2<f64>, csv_path: P) -> io::Result<()> {
    ...
    for row in matrix.rows() {
        wtr.serialize(row.iter().copied().collect::<Vec<f64>>())?;
    }
    wtr.flush()?;
    Ok(())
}
...
/// Prints a heatmap of an `Array2<f64>` to the terminal with Z-normalization
