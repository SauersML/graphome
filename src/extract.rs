...
use crate::eigen_print::{
    ...
    print_heatmap, print_heatmap_ndarray, save_array1_to_csv, save_array2_to_csv,
};

/// Saves an `Array2<f64>` to CSV by delegating to the faer-backed CSV helper.
fn save_ndarray_to_csv<P: AsRef<Path>>(matrix: &Array2<f64>, path: P) -> io::Result<()> {
    let save_start = Instant::now();
    println!(
        "    ⏳ Writing ndarray matrix to CSV at {:?}",
        path.as_ref()
    );
    let write_start = Instant::now();
    save_array2_to_csv(matrix, path)?;
...
pub fn extract_and_analyze_submatrix<P: AsRef<Path> + Send + Sync>(
    ...
    save_array1_to_csv(&eigvals, &eigenvalues_csv)?;
    ...
    save_array2_to_csv(&eigvecs, &eigenvectors_csv)?;
