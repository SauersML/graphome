...
def generate_approx_circulant_graph(n, k, perturbation_prob=0.1):
    """
    Generates a graph with an approximately circulant structure.
    Each node is connected to its k nearest neighbors, and some edges are perturbed randomly.
    
    n: Number of nodes
    k: Number of nearest neighbors to connect to on each side
    ...
    # Add edges to form a circulant structure (k nearest neighbors on both sides)
    for i in range(n):
        for j in range(1, k + 1):
            G.add_edge(i, (i + j) % n)  # Forward neighbor
            G.add_edge(i, (i - j) % n)  # Backward neighbor
...
def main():
    ...
    # Parameters for approximately circulant graph
    n = 1001  # Number of nodes
    k = 2    # Number of nearest neighbors on each side
    perturbation_prob = 0.00  # Probability of perturbing edges
    
    # Generate approximately circulant graph
    G = generate_approx_circulant_graph(n, k, perturbation_prob)
    print(f"Generated approximately circulant graph with {n} nodes.\n")
    ...
    create_2d_plot(eigenvectors_sorted, "Approx Circulant Graph Eigenvectors", "approx_circulant_eigenvectors_2d.png")
